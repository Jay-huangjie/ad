...
import com.zlfcapp.batterymanager.huawei.R
...
abstract class HwProviderSplash : BaseAdProvider(), LifecycleEventObserver {
    ...
    override fun loadAndShowSplashAd(
        ...
        val build = AdSlot.Builder()
            ...
            .disableSdkCountDown(splashSetting.disableSdkCountDown)
        if (splashSetting.width != -1 && splashSetting.height != -1) {
            build.acceptedSize(splashSetting.width, splashSetting.height)
        }
        ...
        adsContext.loadSplashAds(adSlot, object : TemplateLoadListener {
            ...
            override fun onAdLoad(ads: MutableList<ITemplateAd>?) {
                if (ads != null && ads.isNotEmpty()) {
                    ...
                    ad.setInteractionListener(object : InteractionListener {
                        ...
                        override fun onRenderSuccess(view: View?, width: Float, height: Float) {
                            view?.let {
                                container.addView(it, width.toInt(), height.toInt())
                            }
                        }
                    })
