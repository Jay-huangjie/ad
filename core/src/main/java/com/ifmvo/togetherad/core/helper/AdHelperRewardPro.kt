...
object AdHelperRewardPro : BaseHelper() {
    ...
    //为了照顾 Java 调用的同学
    fun show(@NotNull activity: FragmentActivity, @NotNull alias: String, listener: RewardListener? = null) {
        show(activity, alias, null, listener)
    }

    fun show(@NotNull activity: FragmentActivity, @NotNull alias: String, ratioMap: LinkedHashMap<String, Int>? = null, listener: RewardListener? = null) {
        startTimer(listener)
        realShow(activity, alias, ratioMap, listener)
    }

    private fun realShow(@NotNull activity: FragmentActivity, @NotNull alias: String, ratioMap: LinkedHashMap<String, Int>? = null, listener: RewardListener? = null) {
        val currentRatioMap = if (ratioMap?.isEmpty() != false) com.ifmvo.togetherad.core.TogetherAd.getPublicProviderRatio() else ratioMap
