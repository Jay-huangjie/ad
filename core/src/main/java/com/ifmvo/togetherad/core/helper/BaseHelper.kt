...
abstract class BaseHelper {
    ...
    fun filterType(@NotNull ratioMap: LinkedHashMap<String, Int>, adProviderType: String): LinkedHashMap<String, Int> {
        ...
        if (!com.ifmvo.togetherad.core.TogetherAd.failedSwitchEnable) {
    ...
    fun startTimer(listener: BaseListener?) {
        //0 就不开启倒计时
        if (com.ifmvo.togetherad.core.TogetherAd.maxFetchDelay <= 0L) {
            return
        }

        cancelTimer()
        "开始倒计时：${com.ifmvo.togetherad.core.TogetherAd.maxFetchDelay}".logv()
        mTimer = object : CountDownTimer(com.ifmvo.togetherad.core.TogetherAd.maxFetchDelay, 1000) {
